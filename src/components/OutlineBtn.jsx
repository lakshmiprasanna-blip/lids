...
export default function OutlineBtn({
  ...
  children = "View all",
  width,
  height,              // remove default "56px" here
  icon = false,
  className = "",
  bgColor,
  textColor,
  ...
  const style = {
    ...(width && { width }),
    ...(height && { height }),   // now conditional
    ...(bgColor && { backgroundColor: bgColor }),
    ...(textColor && { color: textColor }),
  };
  const sharedClassName = `inline-flex items-center justify-center gap-[10px] whitespace-nowrap border border-[#9E2016] rounded-[65px] ${
    !bgColor ? "text-[#9E2016]" : ""
  ...
  if (onClick) {
    return (
      <button type="button" onClick={onClick} style={style} className={`cursor-pointer ${sharedClassName}`}>
        {children}
  ...
  return (
    <Link href={href} style={style} className={sharedClassName}>
      {children}
      {icon && <ArrowRight size={16} />}
    </Link>
  );
