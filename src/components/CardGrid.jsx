...
export default function CardGrid({ label, title, cards, cols = 3, minHeight = 352, iconSize = 48, titleColor = "#1A1A1A", mobileClassName = "", desktopClassName = "",titleNoWrap = false,titleFontSize = "24px", showBlob = false, blobs = defaultBlobs, bgColor="white",headingAlign="Left" }) {
  return (
...
<div className={`hidden md:grid gap-6 ${desktopClassName} ${cols === 2 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"}`}>
    ...
    return (
      <div
        key={i}
        className={`group ${isLastOdd ? "md:col-span-2 lg:col-span-1 md:justify-self-center lg:justify-self-stretch md:max-w-[calc(50%-12px)] lg:max-w-none w-full" : ""}`}
        style={{ ...cardStyle, minHeight: `${minHeight}px`, border: "1px solid #20B2AA30" }}
      >
        {card.icon && (
