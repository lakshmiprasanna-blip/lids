...
const contentClass = "transition-colors duration-300 group-hover:bg-[#e2f5f3]";
      ...
      <div className="relative z-10 container">
        ...
        {mobileStack && (
          <div className="flex flex-col gap-5 lg:hidden">
            {programs.map((item, i) => (
              <div key={i} className="group w-full rounded-[24px] overflow-hidden bg-white" style={cardGlow}>
                <div className={`${imgWrap} h-[220px] rounded-t-[24px]`}>
                  <Image src={item.image} alt={item.title} fill className={imgClass} />
                  {item.years && (
                ...
                <div className={`${contentClass} rounded-b-[24px] p-5 flex flex-col gap-3`} style={{ border: "1px solid #E5F3F25C", backdropFilter: "blur(14.5px)" }}>
                  <h4 className="text-[#1A1A1A] text-lg font-bold leading-tight">{item.title}</h4>
                  <p className="text-[#7A7A7A] text-sm leading-relaxed">{item.desc}</p>
                  <Link href={item.href || linkHref} className="inline-flex items-center justify-center gap-2 rounded-full text-sm font-medium border border-[#9E2016] text-[#9E2016] hover:bg-[#9E2016] hover:text-white transition-all duration-300 mt-1" style={{ width: "150px", height: "44px" }}>
          ...
          <div className="flex flex-col gap-6 lg:hidden">
            <div className="overflow-hidden">
              <div className="flex transition-transform duration-300 ease-in-out" style={{ transform: `translateX(calc(-${current * 88}vw))` }}>
                {programs.map((item, i) => (
                  <div key={i} className="group rounded-[24px] overflow-hidden shrink-0 mr-4 bg-white" style={{ width: "85vw", ...cardGlow }}>
