...
export default function DentalPrograms({
  ...
  const getCourseHref = (item) => {
    if (item.href) return item.href;
    if (item.slug) return `/coursedetail/${item.slug.split("/").filter(Boolean).pop()}`;
    return linkHref;
  };
...
{showMobileBlobs && (
  <>
      ...
      {programs.map((item, i) => (
        <div key={i} className="group w-full rounded-[24px] overflow-hidden bg-white" style={cardGlow}>
          <div className={`${imgWrap} h-[280px] rounded-t-[24px]`}>
            <Image src={item.image} alt={item.title} fill sizes="(max-width: 1280px) 100vw, 33vw" className={imgClass} />
          ...
          <div className={`${contentClass} h-[280px] rounded-b-[24px] p-6 flex flex-col justify-between`} style={{ border: "10px solid #E5F3F25C" }}>
            ...
            <OutlineBtn href={getCourseHref(item)} width="159px" height="48px" icon={false}>{buttonText}</OutlineBtn>
          </div>
        </div>
      ))}
    </div>

    {/* Stacked list: mobile/tablet when mobileStack=true (no carousel, no drag) */}
    {mobileStack && (
      <div className="flex flex-col gap-6 xl:hidden">
        {programs.map((item, i) => (
          <div key={i} className="group w-full rounded-[24px] overflow-hidden bg-white" style={cardGlow}>
            <div className={`${imgWrap} h-[240px] rounded-t-[24px]`}>
              <Image src={item.image} alt={item.title} fill sizes="100vw" className={imgClass} />
              <div className="absolute top-5 left-5 bg-[#FFFFFFCC] rounded-full px-4 py-1" style={{ border: "2px solid #9E20165E" }}>
                <span className="text-[#9E2016] text-[14px] font-medium">{item.years}</span>
              </div>
            </div>
            <div className={`${contentClass} rounded-b-[24px] p-6 flex flex-col gap-3`} style={{ border: "10px solid #E5F3F25C" }}>
              <h4 className="card-title text-lg font-semibold">{item.title}</h4>
              <p className="text-[#7A7A7A] text-md leading-relaxed">{item.desc}</p>
              <OutlineBtn href={getCourseHref(item)} width="159px" height="48px" icon={false}>{buttonText}</OutlineBtn>
            </div>
    ...
    {/* Carousel: below xl (mobile, tablet, 1024px) — only when mobileStack is false */}
    {!mobileStack && (
      <div className="flex flex-col gap-6 xl:hidden">
            ...
            {programs.map((item, i) => (
              ...
              >
                ...
                <div className={`${contentClass} h-[265px] md:h-[220px] rounded-b-[16px] p-6 flex flex-col justify-between`} style={{ background: "linear-gradient(180deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.16) 100%)", border: "1px solid #A5E7F07A", backdropFilter: "blur(14.5px)", WebkitBackdropFilter: "blur(14.5px)", boxShadow: "inset 0px 2px 12px 0px #FFFFFF40, 0px 6px 6px 0px #EBFDFF33, 0px 13px 13.9px 0px #C9F9FF33" }}>
                  ...
                  <Link href={getCourseHref(item)} className="inline-flex items-center justify-center gap-2 rounded-full text-sm font-medium" style={{ width: "159px", height: "48px", border: "1px solid #9E2016", color: "#9E2016" }}>
...
export function NewsEvents({ events = [], buttonText = "View all" }) {
  ...
  <div className="absolute pointer-events-none z-0 rounded-full" style={{ width: "417px", height: "400px", top: "569px", left: "1226px", background: "#ffffff", opacity: 0.51, filter: "blur(186px)", }}/>      <div className="relative z-10 container">
        ...
        <div className="hidden lg:block">
          <div className="relative flex items-center px-2">
            <CarouselArrowButton direction="prev" onClick={prev} size={56} variant="hover" style={{ position: "absolute", left: "-15px", zIndex: 10 }} />
              ...
              {events.slice(current, current + itemsPerPage).map((item, i) => (
                <div key={item.id ?? i} className="group w-full rounded-[24px] overflow-hidden bg-white" style={cardGlow}>
                  <div className={`${imgWrap} h-[220px] xl:h-[260px] rounded-t-[24px] bg-[#F2F2F2]`}>
                    {item.image && (
                      <Image src={item.image} alt={item.title} fill sizes="(max-width: 1024px) 100vw, 33vw" className={imgClass} />
            ...
            <CarouselArrowButton direction="next" onClick={next} size={56} variant="hover" style={{ position: "absolute", right: "-15px", zIndex: 10 }} />
          </div>
        </div>
