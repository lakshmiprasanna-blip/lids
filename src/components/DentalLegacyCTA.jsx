  ...
  return (
    <section className="relative w-full h-[440px] md:h-[560px] overflow-hidden flex items-center justify-center">
      ...
      <div className="md:hidden absolute inset-0">
        <Image src={mobileImage || image} alt={title} fill priority={priority} sizes="100vw" className="object-cover"style={{ zIndex: 0, objectPosition: mobileImagePosition, ...mobileImageStyle }}/>
      </div>
      {/* Desktop image */}
      <div className="hidden md:block absolute inset-0">
        <Image src={image} alt={title} fill priority={priority} className="object-cover" style={{ zIndex: 0, ...imageStyle }}/>
      </div>
      <div className="absolute inset-0" style={{ background: "linear-gradient(90deg, rgba(32,178,170,0.8) 0%, rgba(16,123,113,0.8) 100%)", zIndex: 1 }} />
      <div className={`container relative flex flex-col gap-6 px-6 w-full mx-auto items-center text-center ${   isLeft ? "md:items-start md:text-left" : "" }`}
        style={{ zIndex: 2 }}
      >
