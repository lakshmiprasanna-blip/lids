...
function FacilitiesBlobs({ blobColor = "#CFEFED", singleBlob = false }) {
  if (singleBlob) {
    return (
...
export default function FacilitiesOverview({
  ...
  return (
    <section className="relative w-full py-25">
      <div className="relative container">
       <FacilitiesBlobs blobColor={blobColor} />
  ...
  <p className="mt-4 text-[#333333] text-[18px] max-w-[1062px] mx-auto">{description}</p>
        ...
        <div className="relative z-10 md:hidden flex items-center justify-center gap-6 p-5">
          <button onClick={prev} className={navBtn}><ArrowLeft size={20} /></button>
          <Link href="/facilities" className="text-[#9E2016] text-md font-medium border border-[#9E2016] rounded-[65px] hover:bg-[#9E2016] hover:!text-white transition-all duration-200 flex items-center justify-center flex-shrink-0" style={{ width: "159px", height: "48px" }}> Explore More
          </Link>
