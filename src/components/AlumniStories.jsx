  ...
  return (
    <section className="w-full bg-white py-20 overflow-hidden">
      <div className="container text-center mb-16">
        <p className="text-[#9E2016] text-md font-semibold uppercase mb-4">ALUMNI STORIES</p>
      ...
      <div className="md:hidden flex flex-col gap-6 px-5">
        <div className="flex flex-col justify-between p-6 rounded-[24px]" style={{ minHeight: "280px", border: "1px solid #A5E7F07A", boxShadow: "inset 0px 2px 12px 0px #FFFFFF40, 0px 22px 9px 0px #D8D8D803" }}>
      ...
      <div className="hidden md:block relative w-full overflow-hidden" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
        <div ref={trackRef} className="flex gap-6" style={{ width: "max-content", animation: "marquee 30s linear infinite", animationPlayState: paused ? "paused" : "running" }}>
          {[...reviews, ...reviews].map((r, i) => (
           <div key={i} className="group flex flex-col justify-center p-10 rounded-[24px] shrink-0 bg-[#FFFFFF1A] hover:bg-[#e9faf8] transition-colors duration-300" style={{ width: "708px", minHeight: "322px", border: "1px solid #A5E7F07A", boxShadow: "inset 0px 2px 12px 0px #FFFFFF40, 0px 1px 3px 0px #FAFAFA0A, 0px 6px 6px 0px #F6F6F608, 0px 13px 8px 0px #EEEEEE05, 0px 22px 9px 0px #D8D8D803" }}>
              <p className="text-[#3D3D3D] group-hover:text-black text-base leading-relaxed italic">{r.quote}</p>
              <div className="flex items-center gap-3 mt-6">
              <div className="relative w-18 h-18 rounded-full overflow-hidden shrink-0">
              <Image src={r.image} alt={r.name} fill className="object-cover" />
              </div>
            <div>
                <p className="text-[#1A1A1A] group-hover:text-[#8E0019] text-xl font-semibold">{r.name}</p>
      ...
      <div className="flex justify-center mt-12">
        <Link href="/alumni" className="text-[#9E2016] text-sm font-medium border border-[#9E2016] rounded-[65px] hover:bg-[#9E2016] hover:!text-white transition-all duration-200 flex items-center justify-center" style={{ width: "144px", height: "52px" }}>
          View all
        </Link>
      </div>
