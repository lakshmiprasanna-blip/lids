...
function ImageCarousel({ images, current, setCurrent }) {
  return (
    <div className="relative w-full h-[240px] md:h-[420px] rounded-2xl overflow-hidden">
      <SafeImage src={images[current]} alt={`Slide ${current + 1}`} fill className="object-cover transition-all duration-500" />
...
function HODCard({ hod }) {
  return (
    <>
      <div className="hidden md:flex items-stretch" style={{ background: "#107B71", borderRadius: "24px", border: "1px solid rgba(255,255,255,0.2)", padding: "44px", gap: "20px", minHeight: "352px" }}>
  <div className="flex flex-col gap-3 justify-center" style={{ flex: "1 1 0" }}>
    <h4 className="!text-white font-semibold !text-[32px]">HOD Message</h4>
    <p style={{ color: "rgba(255,255,255,0.92)", fontSize: "18px", lineHeight: "1.7" }}>
      {hod.message}
    </p>
  </div>
  <div className="shrink-0 relative self-start" style={{ width: "360px", height: "360px", borderRadius: "14px", border: "3px solid rgba(255,255,255,0.3)", overflow: "hidden" }}>
    <SafeImage src={hod.image} alt={hod.name || "HOD"} fill className="object-cover object-top" />
      ...
      <div className="hidden md:grid gap-3" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
        {items.slice(current, current + cols * (h === "180px" ? 2 : 1)).map((item, i) => (
          <div key={i} className="rounded-2xl overflow-hidden" style={{ border: "1px solid #E5F3F2" }}>
            ...
            {item.name && <div className="px-3 py-4 bg-white"><p className="text-[#1A1A1A] font-semibold text-[24px]">{item.name}</p>
            <p className="text-[#6B6B6B] py-2 text-[18px]">{item.dept}</p></div>}
          </div>
        ))}
      </div>
    </>
...
function StatsCarousel({ stats }) {
  ...
  return (
    <>
      ...
      <div className="hidden md:grid" style={{ gridTemplateColumns: `repeat(${stats.length}, 1fr)`, borderRadius: "16px", border: "2px solid #E5F3F2A1", padding: "28px 32px", background: "#FFFFFF", boxShadow: "inset 0 0 10px 10px rgba(207, 239, 237, 0.9)", gap: "20px" }}>
        {stats.map((stat, i) => (
...
// Shared dropdown panel used by both search results and the category list
function DropdownPanel({ items, onSelect, emptyText, className = "" }) {
  ...
  return (
    <div className={`absolute top-full mt-2 bg-white rounded-2xl shadow-lg z-50 overflow-hidden max-h-[320px] overflow-y-auto ${className}`}>
      {items.length > 0
        ? items.map((c) => (
            <button key={c.slug} onClick={() => onSelect(c.slug)} className="w-full text-left px-4 py-3 text-sm text-[#3D3D3D] hover:bg-[#F5FFFE] border-b border-[#F0F0F0] last:border-b-0">
              {c.title}
            </button>
          ))
...
export default function CoursePage({ course, allCourses = [] }) {
  ...
  useEffect(() => {
    const close = (e) => {
      if (!searchRef.current?.contains(e.target) && !categoryRef.current?.contains(e.target)) {
 ...
 const getMax = (s) => {
        ...
        <div className="mx-auto flex items-center justify-center gap-3 flex-wrap" style={{ maxWidth: "1420px", minHeight: "48px" }}>
          <div ref={searchRef} className="relative w-full max-w-[600px]">
            <div className="flex items-center gap-2 bg-white rounded-full px-3 !py-4 w-full">
              ...
              <input
                ...
                placeholder="Search..."
                value={search}
                onChange={(e) => { setSearch(e.target.value); setOpenDropdown("search"); }}
                onFocus={() => search && setOpenDropdown("search")}
                className="text-sm text-[#3D3D3D] outline-none w-full bg-transparent placeholder:text-[#9A9A9A]"
              />
            </div>
            {openDropdown === "search" && (
              <DropdownPanel items={searchResults} onSelect={goToCourse} emptyText={search.trim() ? "No courses found" : null} className="left-0 right-0" />
            )}
          </div>
          ...
          <div ref={categoryRef} className="relative">
            ...
            {openDropdown === "category" && (
              <DropdownPanel items={otherCourses} onSelect={goToCourse} className="left-0 min-w-[240px]" />
            )}
          </div>
      ...
      <div className="container mx-auto px-4 md:px-6 !py-18 flex items-start gap-0 md:gap-8">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-8">
          <div ref={overviewRef}>
            ...
            <p className="text-[#333333] font-medium text-[18px] mb-6">{course.tagline}</p>
  ...
  <div className="hidden md:flex gap-2 shrink-0">
    <button
      onClick={() => setIdx(i, Math.max(0, getIdx(i) - getStep(section)))}
      className="w-8 h-8 rounded-full flex items-center justify-center cursor-pointer border-[1.5px] border-[#20B2AA] text-[#20B2AA] bg-white hover:bg-[#20B2AA] hover:text-white active:bg-[#20B2AA] active:text-white transition-colors"
    >
      <ArrowLeft size={15} />
    </button>
    <button
      onClick={() => setIdx(i, Math.min(getMax(section), getIdx(i) + getStep(section)))}
      className="w-8 h-8 rounded-full flex items-center justify-center cursor-pointer border-[1.5px] border-[#20B2AA] text-[#20B2AA] bg-white hover:bg-[#20B2AA] hover:text-white active:bg-[#20B2AA] active:text-white transition-colors"
    >
      <ArrowRight size={16} />
    </button>
              ...
              {section.htmlContent && (
                <div
                  className="text-[#333333] text-[18px] leading-relaxed [&_ul]:list-disc [&_ul]:pl-5 [&_strong]:font-semibold [&_h2]:text-2xl [&_h2]:font-semibold [&_h3]:text-xl [&_h3]:font-semibold [&_p]:mb-2"
