    ...
    <main>
      ...
      <section className="relative w-full bg-white !py-20 overflow-hidden">
        ...
        <div className="relative z-10 container !px-2 md:px-0">
          ...
          {facilities.length === 0 ? (
            <p className="text-center text-[#9A9A9A]">No facilities found.</p>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {facilities.map((item) => (
                <div
                  key={item.id}
                  className="group relative flex flex-col items-center justify-center gap-4 rounded-[16px] cursor-pointer overflow-hidden hover:scale-[1.02] transition-transform duration-300 h-[160px] md:h-[320px]"
                  style={cardStyle}
                >
                  <div className="relative z-10 flex flex-col items-center justify-center gap-4 transition-opacity duration-300 group-hover:opacity-0">
                    {item.icon && (
                      <Image src={item.icon} alt={item.label} width={60} height={60} className="object-contain md:w-[80px] md:h-[80px]" />
                    )}
                    <span className="text-[#232323] text-md md:text-[24px] font-medium text-center px-2">{item.label}</span>
                  </div>
                  ...
                  <div className="absolute inset-0 rounded-2xl border-5 border-white/40 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

                  <span className="absolute inset-0 z-20 flex items-center justify-center text-white text-base md:text-[24px] font-semibold underline opacity-0 group-hover:opacity-100 transition-opacity duration-300 px-2 text-center">
                    {item.label}
                  </span>
                </div>
              ))}
